...
function cambiareadonlyborrar(input) 
{
    var checknam = input.value;
    var checkval = checknam.substring(0, checknam.length-2);
    if ($("#"+checknam).attr("checked")){
...
function cambiAttFilDatepick(input) 
{
    var checknam = input.value;
    var checkval = checknam.substring(0, checknam.length-2);
    var fil = checkval.substring(checkval.length-1, checkval.length);
    var valoringresado = $("vigencia_inico" + fil).text();
    if ($("#"+checknam).attr("checked")) {
        $("#"+checkval).removeAttr("readonly");
        $("#vigencia_inicio"+fil).removeAttr("readonly");
        $("#vigencia_inicio"+fil).datepicker("enable");
        $("#vigencia_inicio"+fil).datepicker( {
            closeText: 'Cerrar',
            prevText: '&#x3c;Ant',
            nextText: 'Sig&#x3e;',
            currentText: 'Hoy',
        ...
        $("#vigencia_inicio" + fil).change(function() {
            $("#vigencia_final" + fil).removeAttr("readonly");
            $("#vigencia_final" + fil).datepicker("enable");
            $("#vigencia_final" + fil).datepicker( {
                closeText: 'Cerrar',
                prevText: '&#x3c;Ant',
                nextText: 'Sig&#x3e;',
                currentText: 'Hoy',
        ...
        $("#"+checkval).attr("readonly", true);
        $("#vigencia_inicio"+fil).attr("readonly", true).datepicker("disable");
        $("#vigencia_final"+fil).attr("readonly", true).datepicker("disable");
    }
}
...
function validafechamayor(fechai, fechaf) 
{
	var iMes=fechai.substring(3, 5);
	var iDia=fechai.substring(0, 2);
	var iAnio=fechai.substring(6, 10);
	var fMes=fechaf.substring(3 ,5);
	var fDia=fechaf.substring(0, 2);
	var fAnio=fechaf.substring(6, 10);
	if (iAnio > fAnio) {
		return (true);
	}else{
		if (iAnio == fAnio) {
